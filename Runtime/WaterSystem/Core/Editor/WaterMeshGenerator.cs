...
namespace Snm.WaterSystem.EditorTools
{
    /// <summary>
    /// Generates a water surface mesh by running 2D marching squares on a terrain-height field.
    ///
    /// Pipeline:
        ...
        private static List<TerrainTri> ExtractTerrainTriangles(Input input, out int skippedMeshes)
        {
            ...
            foreach (var go in input.TerrainObjects)
            {
                ...
                foreach (var mf in filters)
                {
                    ...
                    {
                        skippedMeshes++;
                        Debug.LogWarning($"[WaterMeshGenerator] Failed to read mesh '{mesh.name}': {e.Message}");
                    }
                }
            }
            return result;
