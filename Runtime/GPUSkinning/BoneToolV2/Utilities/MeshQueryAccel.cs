...
namespace Snm.GPUSkinning.BoneToolV2
{
    ...
    public class MeshQueryAccel
    {
        ...
        public void Build(Vector3[] positions, float cellSize)
        {
            ...
            var size = max - min;
            _gridX = Mathf.Max(1, Mathf.CeilToInt(size.x / _cellSize));
            _gridY = Mathf.Max(1, Mathf.CeilToInt(size.y / _cellSize));
            _gridZ = Mathf.Max(1, Mathf.CeilToInt(size.z / _cellSize));
