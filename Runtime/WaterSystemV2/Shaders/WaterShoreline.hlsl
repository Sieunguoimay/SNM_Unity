#ifndef WATER_V2_SHORELINE_INCLUDED
#define WATER_V2_SHORELINE_INCLUDED

// shorelineUV.x = normalized distance to shore (0 at shore, 1 in deep water)
// shorelineUV.y = normalized along-shore arc length (seamless on closed loops)
// Both are baked into UV1 by the Bake Shore Mesh button (WaterShoreBaker).
float ComputeShoreline(float2 shorelineUV)
{
    ...
    for (int i = 0; i < _ShorelineWaveCount; i++)
    {
        ...
        // Sine band rolling from deep water toward shore. The along-shore
        // term wobbles the band so it doesn't read as a parallel stripe.
        ...
        shorelineFoam += wave * (1.0 - shoreDist);
    }

    return saturate(shorelineFoam) * _ShorelineFoamStrength;
}

#endif // WATER_V2_SHORELINE_INCLUDED
