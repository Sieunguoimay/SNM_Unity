...
half4x4 LoadBoneMatFromTexture(uint frame, uint boneIndex)
{
    ...
    uint blockRow = frame / blockCount;
    uint blockCol = frame - blockRow * blockCount;
    int2 uv;
    uv.y = blockRow * _boneTextureBlockHeight;
    uv.x = blockCol * _boneTextureBlockWidth;
    ...
    float2 uvFrame;
    uvFrame.x = (uv.x + 0.5) / (float)_boneTextureWidth;
    uvFrame.y = (uv.y + 0.5) / (float)_boneTextureHeight;
