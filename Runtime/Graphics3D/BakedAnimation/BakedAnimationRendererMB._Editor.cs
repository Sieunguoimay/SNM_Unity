...
namespace Snm.Graphics3D.GPUSkinning
{
    ...
    public class BakedAnimationRendererMBEditor : Editor
    {
            ...
            SerializedProperty bakedDataProp)
        {
            ...
            // Create or reuse a material with the GPU skinning shader
            var existingMat = materialProp.objectReferenceValue as Material;
            var gpuSkinShader = Shader.Find("Custom/GpuSkin");
            if (gpuSkinShader == null)
            {
                Debug.LogWarning("BakeAndSetup: Could not find 'Custom/GpuSkin' shader. Using source material as-is.");
                materialProp.objectReferenceValue = smr.sharedMaterial;
            }
            else if (existingMat != null && existingMat.shader == gpuSkinShader)
            {
                // Already has a correctly-set-up material, keep it
            }
            else
            {
                var mat = new Material(gpuSkinShader);
                mat.name = $"{prefab.name}_BakedSkin";

                // Copy main texture from source material
                var srcMat = smr.sharedMaterial;
                if (srcMat != null)
                {
                ...
                var assetPath = AssetDatabase.GetAssetPath(existingAsset);
                var matPath = assetPath.Replace(".asset", "_Mat.mat");
        ...
        private Bounds OnGetFrameBounds()
        {
            ...
            var m = meshProp.objectReferenceValue as Mesh;
            if (m == null) return new Bounds(renderer.transform.position, Vector3.one);

            var localBounds = m.bounds;
            var t = renderer.transform;
            // Transform local bounds to world space
            var center = t.TransformPoint(localBounds.center);
            var extents = localBounds.extents;
            var worldExtents = new Vector3(
                Mathf.Abs(t.lossyScale.x) * extents.x,
                Mathf.Abs(t.lossyScale.y) * extents.y,
                Mathf.Abs(t.lossyScale.z) * extents.z);
