    ...
    public static class SceneInputHandler
    {
        ...
        public static RigDocument.ToolModeEnum? HandleSceneInput(SceneView view, IToolMode activeMode, RigDocument doc)
        {
            ...
            if (e.type == EventType.KeyDown)
            {
                ...
                // 'F' to frame selected bone (#7)
                if (e.keyCode == KeyCode.F && doc.selectedBoneIndex >= 0 && doc.selectedBoneIndex < doc.bones.Count)
                {
                    var boneWorldPos = doc.GetBoneWorldPosition(doc.selectedBoneIndex);
                    view.Frame(new Bounds(boneWorldPos, Vector3.one * 0.5f), false);
                    e.Use();
                    return null;
                }
