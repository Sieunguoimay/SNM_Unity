...
namespace Snm.Graphics3D.Animation
{
    public class BakedAnimationPreviewWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/Snm/3D Toolkit/Animation/Baked Animation Preview")]
        ...
        private void OnEnable()
        {
            ...
            // Auto-pick target from selection if none set
            if (_target == null)
            {
                var selected = Selection.activeGameObject;
                if (selected != null)
                {
                    var renderer = selected.GetComponent<BakedAnimationRendererMB>();
                    if (renderer != null)
                        SetTarget(renderer);
                }
            }
        ...
        private void OnDisable()
        {
            SceneView.duringSceneGui -= OnSceneGUI;
            EditorApplication.update -= OnEditorUpdate;
            if (_target != null) _target.SuppressRendering = false;
            CleanupPreview();
        }
        ...
        {
            // Restore previous target's rendering
            if (_target != null) _target.SuppressRendering = false;
            ...
            if (_target != null)
            {
                _target.SuppressRendering = true;
                SetupPreview();
            }
            RefreshUI();
        }
        ...
        private void SetupPreview()
        {
            ...
            _mesh = Instantiate(meshField);
            _mesh.name = meshField.name + "_Preview";
            BakedAnimationRendererMB.UploadBoneWeightsToUV(_mesh);
